...
**Resume** is generated by `scripts/make-resume.py` (python-docx). Edit the script, then run it to write a local `NoahMichaelsResume.docx` in the repo root (gitignored). Export the PDF to `public/NoahMichaelsResume.pdf` — only the PDF is committed and served from the site. Example export: `soffice --headless --convert-to pdf --outdir public NoahMichaelsResume.docx`. The title is "UX Engineer."
