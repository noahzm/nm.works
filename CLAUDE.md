...
The site builds to static output and deploys to Cloudflare (deploy state lives in the gitignored `.wrangler/`); `site` is set to `https://nm.works` in `astro.config.mjs`. Two MCP servers are configured in `.mcp.json`: `shadcn` (component registry) and `astro` (hosted Astro docs).
